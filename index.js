...
// @ts-ignore
const blobServiceClient = new BlobServiceClient.fromConnectionString("DefaultEndpointsProtocol=https;AccountName=androidTesting;AccountKey=UUUZBh8ZbvqAFTUkLajjJUgU7lyst9eD7tpK7MlR41TlyiSVsk/5F41J1fx7bF14Mhud+cBozQMe0dCZshEFvA==;EndpointSuffix=core.windows.net");
const accountName = 'androidtesting'
const containerName = "test-adroid";

/**
 * Please note:
 * We need to Read connection string, accountName and containerName from env file
 */

express.post('/blob', async (req, res) => {
  try {
    console.log('req.body', req.body)

    if (!req.body.image_data) {
      // throw error. Image Data is required
    }

    let fileData = req.body.image_data.split(',')[1]
    const fileExtension = req.body.image_data.substring("data:image/".length, req.body.image_data.indexOf(";base64"))

    const data = base64.toByteArray(fileData)
    const containerClient = blobServiceClient.getContainerClient(containerName);
    const blobName = "test_file_" + new Date().getTime() + `.${fileExtension}`;
    const blockBlobClient = containerClient.getBlockBlobClient(blobName);
    const uploadBlobResponse = await blockBlobClient.upload(data, Buffer.byteLength(data));
    console.log('Final Upload Response: ', uploadBlobResponse)
    return res.send({
 ...
 * cURL:
 * 
  curl --location --request POST 'localhost:3000/blob' \
